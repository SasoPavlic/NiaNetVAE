...
Last updated: 2026-09-10, after `metropt_controlled_v7` was accepted.
...
**The production study is complete and accepted.** `metropt_controlled_v7`
reports `validate-study` `valid: true` with all five workflows and zero errors.
Nothing is queued on HPC.

The remaining work is analysis and writing, not computation:

1. **Baseline-convergence diagnostic** (agreed, not yet run). See section 8.
2. **Write the paper** against the v7 tables, which are generated by
   `docs/build_results.py`, never typed.
3. **Multi-seed confidence intervals** remain unaffordable under the current
   contract. See section 8.
...
| `metropt_controlled_v7` | **accepted** | **The production study** |
...
2. **Baseline convergence is untested, and the diagnostic is agreed but not run.**
   Under `patience: 2` and `min_epochs: 3`, cycle-0 training ran:

   | Model | Epochs | Best epoch |
   |---|---|---|
   | `sae_static` | 3 | 1 |
   | NiaNetVAE cycle 0 | 6 | 4 |
   | `vae_static` | 8 | 6 |

   `sae_static` stopped at the floor: its validation loss never improved after
   epoch 1. Note that the contributed method trained **less** than the VAE
   baseline under the identical budget and identical stopping rule, so there is
   **no evidence of bias** toward NiaNetVAE.

   The open question is **power, not bias**: `patience: 2` is aggressive for
   every model, so all three may be uniformly undertrained. The agreed remedy is
   a diagnostic run *outside* the study artifacts — a separate `study_id` with a
   longer patience, running only `sae_static` and `vae_static` — to show whether
   the ordering changes. Do **not** fix this by editing v7; `training.*` sits
   inside the immutable contract and any change forces a new `study_id`.
